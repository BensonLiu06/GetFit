...
# Implementation of Track Progress window
def createTrackProgressWindow(mainWindow, parentWindow, dbConnection, dbCursor, username):
    # Hide the App window
    parentWindow.grid_forget()

    # Create a frame for the Track Progress window
    trackProgressWindow = ttk.Frame(mainWindow, padding=(3,3,12,12))
    trackProgressWindow.grid(sticky = (N, S, E, W))

    # Setup the User Registration window
    trackProgressWindow.columnconfigure(0, weight = 1)
    trackProgressWindow.columnconfigure(1, weight = 1)
    trackProgressWindow.rowconfigure(0, weight = 1)
    trackProgressWindow.rowconfigure(1, weight = 1)

    # Create all the main frame containers
    topFrame = ttk.Frame(trackProgressWindow, width = 600, height = 50, relief = 'groove', borderwidth = 2)
    bottomFrame = ttk.Frame(trackProgressWindow, width = 300, height = 700, relief = 'groove', borderwidth = 2)
    buttonFrame = ttk.Frame(trackProgressWindow, width = 300, height = 50, relief = 'groove', borderwidth = 2)
    ...
    informationLabel =ttk.Label(topFrame, text = "Track your progress")
    ...
    popupBox(mainWindow, trackProgressWindow, "Warning", "Track Progress Window implementation incomplete")
    ...
    cancelButton = ttk.Button(buttonFrame, text = "Cancel", command = lambda : 
                                   cancelTrackProgressWindow(mainWindow, trackProgressWindow, parentWindow))
    cancelButton.grid(column = 1, row = 12, pady = 5)

# Implementation of the Cancel button for the Track Progress window
# Closes the Track Progress window and returns to the App window
def cancelTrackProgressWindow(mainWindow, parentWindow, appWindow):
